...
const user_collection = Models.User;
const society_collection = Models.Society;
const visit_collection = Models.Visit;
...
app.get("/", (req, res) => {
  ...
  visit_collection.findOne((err, pageVisit) => {
    if (!pageVisit) {
      pageVisit = new visit_collection();
    }
    let count = pageVisit?.count ?? 0;

    count++;
    society_collection.find((err, foundSociety) => {
      const societyCount = foundSociety.length;
      const cities = foundSociety.map((society) =>
        society.societyAddress.city.toLowerCase()
      );
      const cityCount = [...new Set(cities)].length;
      user_collection.find((err, foundUser) => {
...
app.get("/loginFailure", (req, res) => {
  const failureMessage =
    "Sorry, entered password was incorrect, Please double-check.";
...
app.get("/residents", (req, res) => {
  if (req.isAuthenticated() && req.user.validation == "approved") {
    ...
    user_collection.find(
      { $and: [{ societyName: userSocietyName }, { validation: "approved" }] },
      (err, foundUsers) => {
        if (!err && foundUsers) {
...
app.get("/bill", (req, res) => {
  if (req.isAuthenticated() && req.user.validation == "approved") {
    user_collection.findById(req.user.id, (err, foundUser) => {
      if (!err && foundUser) {
        society_collection.findOne(
...
app.get("/success", async (req, res) => {
  ...
  user_collection.findOne({ _id: req.user.id }, (err, foundUser) => {
    ...
    foundUser.save(function () {
      const transactionDate = new Date(customer.created * 1000)
        .toLocaleString()
        .split(", ")[0];
      res.render("success", {
...
app.post("/checkout-session", async (req, res) => {
  const session = await stripe.checkout.sessions.create({
    ...
    success_url:
      "https://esociety-fdbd.onrender.com/https://esociety-fdbd.onrender.com//success?session_id={CHECKOUT_SESSION_ID}",
    cancel_url:
      "https://esociety-fdbd.onrender.com/https://esociety-fdbd.onrender.com//bill",
  });
...
app.post("/closeTicket", (req, res) => {
  ...
  user_collection.findById(user_id, (err, foundUser) => {
    if (!err && foundUser) {
      user_collection.updateOne(
        { _id: user_id },
        {
      ...
      if (!err && foundSociety) {
        user_collection.register(
          ...
          function (err, user) {
            if (err) {
              const failureMessage =
                "Sorry, this email address is not available. Please choose a different address.";
  ...
  society_collection.findOne(
    ...
    function (err, result) {
      if (!err && !result) {
          ...
          (err, user) => {
            if (err) {
              ...
              passport.authenticate("local")(req, res, function () {
                ...
                const society = new society_collection({
        ...
        const failureMessage =
          "Sorry, society is already registered, Please double-check society name.";
